...
class HyperParameter:
    def __init__(self):
        ...
        self.kfold = 5
        self.data_root = '/Users/adele/Documents/GitHub/LLMDTA/data'   #'/homeb/tangwuguo/dataset/dta-cluster'  # /homeb/tangwuguo/dataset  /homeb/tangwuguo/dataset/dta-cluster
        self.dataset = 'davis_b3'      # davis kiba metz
        #self.running_set = 'novel-prot'   # warm novel-drug novel-prot novel-pair
        #self.dataset_columns = ['drug_id', 'prot_id', 'label']
        self.dataset_columns = ['drug_id','compound_iso_smiles','protein_id','target_sequence','affinity']
        self.is_esm=True
        
        # "center_emb", "emb_length", "norm_emb": 
        # ['dataset', 'vec_dict', 'mat_dict', 'length_dict']
        self.mol2vec_dir = f'data/{self.dataset}/{self.dataset}_mol_pretrain.pkl'    #300 384 _drug_pretrain.pkl  _chemBERTa.pkl
        self.protvec_dir = f'data/{self.dataset}/{self.dataset}_esm_pretrain.pkl'  #100 1280 _prot_pretrain.pkl  _esm_pretrain.pkl           
        self.drugs_dir = f'data/{self.dataset}/{self.dataset}_drugs.csv'   
        self.prots_dir = f'data/{self.dataset}/{self.dataset}_prots.csv'   
