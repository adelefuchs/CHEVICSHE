...
def my_collate_fn(
    batch_data, device, hp, drug_df, prot_df, mol2vec_dict, protvec_dict, isEsm=False
):
    ...
    b_drug_mat = torch.zeros(
        (batch_size, drug_substruc_max, mol2vec_dim), dtype=torch.float32
    )
    b_prot_mat = torch.zeros(
        (batch_size, protein_max, protvec_dim), dtype=torch.float32
    )
    # label
    b_label = torch.zeros(batch_size)

    # New: Store IDs as lists of strings
    b_drug_ids = []
    b_prot_ids = []

    for i, pair in enumerate(batch_data):
        # CHANGED THIS NOW THAT MY FILES HAVE MORE COLUMNS
        drug_id, prot_id, label = pair.iloc[-5], pair.iloc[-3], pair.iloc[-1]
        ...
        # Add IDs to list
        b_drug_ids.append(drug_id)
        b_prot_ids.append(prot_id)
