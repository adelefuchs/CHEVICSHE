...
def mol_to_graph(mol, times=2):
    ...
    molGraph.add_nodes_from(
        range(mol.GetNumAtoms())
    )  # ensures all atoms are present as nodes
...
class GNNDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if "train" in self.dataset_name:
            data_list = self.process_train_data(input_path)
        else:
            data_list = self.process_data(input_path)
        print(f"Processed {self.dataset_name}, saving to file...")
...
if __name__ == "__main__":
    # GNNDataset('data/data')
    for i in range(1, 6):
        GNNDataset(root="data", dataset_name=f"davis_b3_train_{i}")
        GNNDataset(root="data", dataset_name=f"davis_b3_test_{i}")

    # Optionally process combined or Pharos files too
    GNNDataset(root="data", dataset_name="pharos")
